...
type ProjectFilter = "needs-review" | "reviewed";
...
export default function Admin() {
  ...
  const [projectFilter, setProjectFilter] = useState<ProjectFilter>("needs-review");
  ...
  useEffect(() => {
    if (!slackId) { setChecking(false); return; }
  ...
  const handleAwardCredits = async () => {
    ...
    if (data.success) {
      ...
      setAwardSlackId(""); setAwardAmount("");
      fetch("/api/admin-get-users", { method: "GET", headers: { "x-slack-id": slackId } })
  ...
  if (!slackId) {
    window.location.href = "https://auth.hackclub.com/oauth/authorize?client_id=c89f85642fe94c65cbead982b0b7e9b8&redirect_uri=http://automake.dino.icu/auth&response_type=code&scope=profile%20email%20name%20slack_id%20verification_status";
    return null;
  }

  if (!authed) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center px-6" style={{ background: "#0F1923" }}>
        <div className="rounded-xl p-10 max-w-md w-full" style={{ border: "2px solid rgba(255,87,51,0.3)", background: "rgba(255,87,51,0.05)" }}>
          <p className="font-sans text-xs font-bold uppercase tracking-[0.25em] mb-4" style={{ color: "#FF5733" }}>Access Denied</p>
          <h1 className="font-sans text-2xl font-extrabold mb-3" style={{ color: "#F5F0E8" }}>You are not authorized.</h1>
          <p className="font-sans text-sm mb-8" style={{ color: "rgba(245,240,232,0.5)" }}>This page is restricted to Automake admins only. If you think this is a mistake, contact @Oba.</p>
          <button onClick={() => setLocation("/")} className="font-sans text-sm font-bold px-6 py-3 rounded-lg cursor-pointer" style={{ background: "#F5F0E8", color: "#0F1923" }}>Back to Home</button>
        </div>
      </div>
    );
  }
  ...
  // Filter out Unsubmitted, then split into needs review vs reviewed
  const submittedProjects = projects.filter((p) => p.status !== "Unsubmitted");
  const needsReview = submittedProjects.filter((p) => p.status === "Pending Review");
  const reviewed = submittedProjects.filter((p) => p.status === "Accepted" || p.status === "Rejected");
  const visibleProjects = projectFilter === "needs-review" ? needsReview : reviewed;
    ...
    <div className="min-h-screen" style={{ background: "#0F1923" }}>
      <div style={{ borderBottom: "1px solid rgba(245,240,232,0.08)" }}>
        <div className="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
          ...
          <button onClick={() => setLocation("/dashboard")} className="font-sans text-sm font-semibold px-4 py-2 rounded-lg cursor-pointer" style={{ background: "rgba(245,240,232,0.06)", color: "rgba(245,240,232,0.6)", border: "1px solid rgba(245,240,232,0.1)" }}>
            Back to Dashboard
          </button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 pt-8">
        <div className="flex gap-2 mb-8">
          {tabs.map((t) => (
            <button key={t.key} onClick={() => { setTab(t.key); setError(null); setAwardMsg(null); }} className="font-sans text-sm font-bold px-5 py-2.5 rounded-lg cursor-pointer transition-all" style={{ background: tab === t.key ? "#00E5A0" : "rgba(245,240,232,0.06)", color: tab === t.key ? "#0F1923" : "rgba(245,240,232,0.6)", border: tab === t.key ? "none" : "1px solid rgba(245,240,232,0.1)" }}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {tab === "projects" && !loading && (
          <div className="pb-16">
            ...
            <div className="flex items-center gap-2 mb-6">
              <button
                onClick={() => setProjectFilter("needs-review")}
                className="font-sans text-sm font-bold px-4 py-2 rounded-lg cursor-pointer transition-all flex items-center gap-2"
                style={{ background: projectFilter === "needs-review" ? "rgba(255,193,7,0.2)" : "rgba(245,240,232,0.06)", color: projectFilter === "needs-review" ? "#856404" : "rgba(245,240,232,0.5)", border: projectFilter === "needs-review" ? "1px solid rgba(255,193,7,0.3)" : "1px solid rgba(245,240,232,0.1)" }}
              >
                Needs Review
                <span className="font-sans text-xs font-extrabold px-2 py-0.5 rounded-full" style={{ background: "rgba(255,193,7,0.2)", color: "#856404" }}>
                  {needsReview.length}
                </span>
              </button>
              <button
                onClick={() => setProjectFilter("reviewed")}
                className="font-sans text-sm font-bold px-4 py-2 rounded-lg cursor-pointer transition-all flex items-center gap-2"
                style={{ background: projectFilter === "reviewed" ? "rgba(245,240,232,0.1)" : "rgba(245,240,232,0.06)", color: projectFilter === "reviewed" ? "rgba(245,240,232,0.8)" : "rgba(245,240,232,0.5)", border: projectFilter === "reviewed" ? "1px solid rgba(245,240,232,0.2)" : "1px solid rgba(245,240,232,0.1)" }}
              >
                Reviewed
                <span className="font-sans text-xs font-extrabold px-2 py-0.5 rounded-full" style={{ background: "rgba(245,240,232,0.1)", color: "rgba(245,240,232,0.6)" }}>
                  {reviewed.length}
                </span>
              </button>
            </div>
            ...
            <div className="flex flex-col gap-4">
              {visibleProjects.length === 0 && (
                <p className="font-sans text-sm" style={{ color: "rgba(245,240,232,0.4)" }}>
                  {projectFilter === "needs-review" ? "No projects waiting for review." : "No reviewed projects yet."}
                </p>
              )}
              {visibleProjects.map((p) => {
                const statusStyle = STATUS_COLORS[p.status] || STATUS_COLORS["Unsubmitted"];
                return (
                  <motion.div key={p.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl p-6" style={{ background: "rgba(245,240,232,0.04)", border: "1px solid rgba(245,240,232,0.08)" }}>
                    <div className="flex items-start justify-between gap-4 mb-3">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <button onClick={() => setLocation(`/admin/project/${p.id}`)} className="font-sans text-xs font-bold px-3 py-1.5 rounded-lg cursor-pointer" style={{ background: "rgba(245,240,232,0.08)", color: "rgba(245,240,232,0.7)", border: "1px solid rgba(245,240,232,0.12)" }}>
                          View
                        </button>
                        <span className="font-sans text-xs font-bold px-3 py-1 rounded-full" style={statusStyle}>
                          {p.status}
                        </span>
                      </div>
                    </div>
                    ...
                    {p.status === "Pending Review" && (
                      <div className="flex items-center gap-3 mt-4">
                        <input
                          type="number" min={0} placeholder="Credits to award"
                        ...
                        />
                        <button onClick={() => handleProjectAction(p.id, "approve")} className="font-sans text-sm font-bold px-5 py-2 rounded-lg cursor-pointer" style={{ background: "#00E5A0", color: "#0F1923" }}>
                          Approve
                        </button>
                        <button onClick={() => handleProjectAction(p.id, "reject")} className="font-sans text-sm font-bold px-5 py-2 rounded-lg cursor-pointer" style={{ background: "rgba(255,87,51,0.15)", color: "#FF5733", border: "1px solid rgba(255,87,51,0.3)" }}>
                          Reject
                        </button>
                      </div>
                    )}
          ...
          <div className="pb-16">
            <div className="rounded-xl p-6 mb-8" style={{ background: "rgba(245,240,232,0.04)", border: "1px solid rgba(245,240,232,0.08)" }}>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <input type="text" placeholder="Slack ID" value={awardSlackId} onChange={(e) => setAwardSlackId(e.target.value)} className="font-sans text-sm px-3 py-2 rounded-lg" style={{ background: "rgba(245,240,232,0.06)", border: "1px solid rgba(245,240,232,0.15)", color: "#F5F0E8", outline: "none", width: "200px" }} />
                <input type="number" min={1} placeholder="Amount" value={awardAmount} onChange={(e) => setAwardAmount(e.target.value)} className="font-sans text-sm px-3 py-2 rounded-lg w-32" style={{ background: "rgba(245,240,232,0.06)", border: "1px solid rgba(245,240,232,0.15)", color: "#F5F0E8", outline: "none" }} />
                <button onClick={handleAwardCredits} className="font-sans text-sm font-bold px-5 py-2 rounded-lg cursor-pointer" style={{ background: "#00E5A0", color: "#0F1923" }}>Award</button>
              </div>
